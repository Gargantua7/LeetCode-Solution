...
// 
//
// Related Topics 贪心 数组 字符串 动态规划 👍 130 👎 0
...
class Solution {
    fun minCost(colors: String, neededTime: IntArray): Int {
        ...
        colors.forEachIndexed { index, c ->
            val time = neededTime[index]

            if (currColor != c) {
                res += currColorSum - currColorMax
                currColor = c
                currColorSum = time
                currColorMax = time
            } else {
                currColorSum += time
                currColorMax = maxOf(time, currColorMax)
            }
        }

        return res + currColorSum - currColorMax
    }
}
