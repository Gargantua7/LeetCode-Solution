...
// 
//
// Related Topics 栈 贪心 数组 动态规划 单调栈 👍 118 👎 0
...
class Solution {
    fun minNumberOperations(target: IntArray): Int {
        ...
        target.forEachIndexed { index, value ->
            ...
            if (target.isLower(index)) {
                lower = value
            } else if (target.isUpper(index)) {
                ans += value - lower
                lower = value
            }
        }
    ...
    fun IntArray.isLower(index: Int): Boolean {
        return this[index] < this.getOrElse(index - 1) { 0 } &&
                this[index] <= this.getOrElse(index + 1) { 0 }
    }

    fun IntArray.isUpper(index: Int): Boolean {
        return this[index] > this.getOrElse(index - 1) { 0 } &&
                this[index] >= this.getOrElse(index + 1) { 0 }
    }
}
