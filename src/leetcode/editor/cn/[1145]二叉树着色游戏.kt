    ...
    fun btreeGameWinningMove(root: TreeNode?, n: Int, x: Int): Boolean {
        val target = run {
            ...
            while (queue.isNotEmpty()) {
                val node = queue.poll()
                if (node.`val` == x) return@run node
                node.left?.let { queue.offer(it) }
                node.right?.let { queue.offer(it) }
            }
            null
        }
        val left = treeNodeCount(target?.left)
        val right = treeNodeCount(target?.right)
        val parent = n - left - right - 1
        val max = maxOf(parent, maxOf(left, right))
        return max > n / 2
    }

    fun treeNodeCount(root: TreeNode?): Int {
        root?: return 0
        val queue = LinkedList<TreeNode>()
        queue.offer(root)
        var count = 0
