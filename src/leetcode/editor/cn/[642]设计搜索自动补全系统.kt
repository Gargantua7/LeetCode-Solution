...
//
// 
// Related Topics 设计 字典树 字符串 数据流 👍 106 👎 0
...
class DesignSearchAutocompleteSystem {
    ...
    class AutocompleteSystem(sentences: Array<String>, times: IntArray) {
        ...
        fun insert(word: String, times: Int = 1) {
            var temp = root
            word.forEach { c ->
                if (c != '#') {
                    if (temp.children[c] == null) temp.children[c] = TrieNode(c)
        ...
        fun search(word: String): List<String> {
            var temp = root
            word.forEach { c ->
                if (temp.children[c] == null) return emptyList()
                temp = temp.children[c]!!
            }
            val res = ArrayList<Pair<String, Int>>()
            dfs(temp, res, StringBuilder(word))
            return res.sortedWith(kotlin.Comparator { (s1, t1), (s2, t2) ->
                if (t1 != t2) t2 - t1 else s1.compareTo(s2)
            }).filterIndexed { i, _ -> i < 3 }.map { it.first }
        }

        fun dfs(node: TrieNode, queue: ArrayList<Pair<String, Int>>, sb: StringBuilder) {
            if (node.times > 0) queue.add(sb.toString() to node.times)
