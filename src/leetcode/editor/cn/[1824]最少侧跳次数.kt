...
// 
//
// Related Topics 贪心 数组 动态规划 👍 107 👎 0
...
class Solution {
    fun minSideJumps(obstacles: IntArray): Int {
        val arr = Array(2) { IntArray(3) { 500001 } }
        arr[0] = intArrayOf(1, 0, 1)
        for (i in 1..obstacles.lastIndex) {
            for (j in 0..2) {
                if (obstacles[i] == j + 1) continue
                arr[1][j] = minOf(arr[0][j], minOf(arr[1][(j + 1) % 3], arr[1][(j + 2) % 3]) + 1)
                if (obstacles[i] != (j + 1) % 3 + 1) arr[1][(j + 1) % 3] = minOf(arr[1][(j + 1) % 3], arr[1][j] + 1)
                if (obstacles[i] != (j + 2) % 3 + 1) arr[1][(j + 2) % 3] = minOf(arr[1][(j + 2) % 3], arr[1][j] + 1)
            }
            arr[0] = arr[1]
            arr[1] = IntArray(3) { 500001 }
        }
        return arr[0].min()!!
    }
}
