...
// 
// Related Topics 深度优先搜索 图 拓扑排序 数组 👍 133 👎 0
...
class LoudAndRich {
    ...
    class Solution {
        fun loudAndRich(richer: Array<IntArray>, quiet: IntArray): IntArray {
            ...
            richer.forEach { (rich, poor) ->
                if (poor !in adjacency) {
                    adjacency[poor] = ArrayList()
                }
                adjacency[poor]!!.add(rich)
            }
            val ans = Array(quiet.size) { it to -1 }
            for (i in ans.indices) {
                dfs(adjacency, i, ans, quiet)
            }
            return ans.map { it.first }.toIntArray()
        }

        fun dfs(
            adjacency: HashMap<Int, out List<Int>>,
            position: Int,
            ans: Array<Pair<Int, Int>>,
            quite: IntArray
        ): Pair<Int, Int> {
            if (ans[position].second > -1) return ans[position]
            var min = position to quite[position]
            adjacency[position]?.forEach { next ->
                val temp = dfs(adjacency, next, ans, quite)
