...
// 
// Related Topics 贪心 数组 双指针 排序 👍 142 👎 0
...
class Solution {
    fun pancakeSort(arr: IntArray): List<Int> {
        val index = IntArray(arr.size + 1)
        val res = ArrayList<Int>()
        arr.forEachIndexed { idx, n -> index[n] = idx }
        for (i in arr.lastIndex downTo 1) {
            val t = index[i + 1]
            if (t == i) continue
            for (j in 1..index.lastIndex) {
                if (index[j] > i) continue
                index[j] = if (index[j] <= t) index[j] + i - t else i - index[j]
            }
            res.add(t + 1)
            res.add(i + 1)
        }
