...
//leetcode submit region begin(Prohibit modification and deletion)
    ...
    class Solution {
        ...
        fun insert(head: Node?, insertVal: Int): Node? {
            val target = Node(insertVal)
            if ((head ?: return target.apply { next = this }) == head.next) {
                head.next = target
                target.next = head
                return head
            }
            var max: Node? = head.next
            while (max!!.next!!.`val` >= max.`val` && max != head) {
                max = max.next
            }
            if (insertVal >= max.`val`) {
                target.next = max.next
                max.next = target
                return head
            }
            var p = max.next
