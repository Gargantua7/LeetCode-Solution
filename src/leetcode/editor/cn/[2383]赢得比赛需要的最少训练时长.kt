...
class Solution {
    fun minNumberOfHours(initialEnergy: Int, initialExperience: Int, energy: IntArray, experience: IntArray): Int {
        val needEnergy = maxOf(energy.sum() - initialEnergy + 1, 0)
        var needExp = 0
        experience.reversed().forEach { n ->
            needExp = if (n >= needExp) n
            else maxOf(n, needExp - n)
        }
        return needEnergy + maxOf(needExp - initialExperience + 1, 0)
    }
}
