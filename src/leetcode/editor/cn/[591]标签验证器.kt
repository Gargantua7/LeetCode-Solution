...
class Solution {
    ...
    companion object {
        ...
        const val UNKNOWN_TAG = 0
        const val OPEN_TAG = 1
        const val CLOSE_TAG = 2
        const val CONTENT = 3
        const val CDATA_OPEN_TAG = 4
        const val CDATA_CONTENT = 6
        const val UNKNOWN_NEXT = 7
    ...
    fun isValid(code: String): Boolean {
        ...
        while (i < code.length) {
            when (curr) {
                ...
                OPEN_TAG -> {
                    if (code[i] == '>') {
                        if (cache.length !in 1..9) return false
                        stack.push(cache.toString())
                        cache.clear()
                        curr = CONTENT
                    } else if (code[i].isUpperCase()) {
                        cache.append(code[i])
                    } else return false
                    i++
                }
                CLOSE_TAG -> {
                    if (code[i] == '>') {
                        ...
                        val top = stack.pop()
                        if (top != cache.toString()) return false
                        cache.clear()
                        curr = UNKNOWN_NEXT
                    } else if (code[i].isUpperCase()) {
                        cache.append(code[i])
                ...
                UNKNOWN_NEXT -> {
                    if (stack.isNotEmpty()) curr = CONTENT
                    else return false
                }
                CDATA_OPEN_TAG -> {
                    if (stack.isEmpty()) return false
                    if (i + 6 in code.indices && code.subSequence(i..i + 6) == "[CDATA[") {
                        i += 7
                        curr = CDATA_CONTENT
                    } else return false
                }
                CDATA_CONTENT -> {
                    if (code[i] == ']' && i + 2 in code.indices && code.subSequence(i..i + 2) == "]]>") {
                        i += 3
                        curr = CONTENT
                    } else i++
                }
        ...
        return stack.isEmpty() && cache.isEmpty() && curr == UNKNOWN_NEXT
    }
}
