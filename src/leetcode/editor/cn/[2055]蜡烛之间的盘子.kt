...
// 
// Related Topics 数组 字符串 二分查找 前缀和 👍 17 👎 0
    ...
    class Solution {
        fun platesBetweenCandles(s: String, queries: Array<IntArray>): IntArray {
            ...
            s.forEachIndexed { i, c ->
                if (c == '|') map[i] = count
                else if (map.isNotEmpty()) count++
            }
            return queries.map { (s, e) ->
                val start = map.ceilingEntry(s) ?: return@map 0
                val end = map.floorEntry(e) ?: return@map 0
                if (start.key >= end.key) 0 else end.value - start.value
            }.toIntArray()
        }
    }
