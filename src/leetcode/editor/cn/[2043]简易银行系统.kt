...
// 
// Related Topics 设计 数组 哈希表 模拟 👍 10 👎 0
...
class SimpleBankSystem {
    ...
    class Bank(balance: LongArray) {
        ...
        val balance: LongArray

        init {
            this.balance = LongArray(balance.size + 1) {
                if (it == 0) 0 else balance[it - 1]
            }
        }

        fun transfer(account1: Int, account2: Int, money: Long): Boolean {
            if (!checkAccountIsValid(account1) || !checkAccountIsValid(account2) ||
                !checkAccountBalanceIsEnough(account1, money)
            ) return false
        ...
        fun deposit(account: Int, money: Long): Boolean {
            if (!checkAccountIsValid(account)) return false
            balance[account] += money
            return true
        }

        fun withdraw(account: Int, money: Long): Boolean {
            if (!checkAccountIsValid(account) || !checkAccountBalanceIsEnough(account, money)) return false
            balance[account] -= money
            return true
        }

        private fun checkAccountIsValid(account: Int) = account in 1..balance.size

        private fun checkAccountBalanceIsEnough(account: Int, money: Long) = balance[account] >= money
