...
class Solution {
    fun minOperationsMaxProfit(customers: IntArray, boardingCost: Int, runningCost: Int): Int {
        ...
        var ans = -1
        var time = 0
        while (time in customers.indices || waitCustomer != 0) {
            if (time in customers.indices) {
                val comeCustomer = customers[time]
                waitCustomer += comeCustomer
            }
            time++
            val playCustomer = minOf(waitCustomer, 4)
            waitCustomer -= playCustomer
            profit += boardingCost * playCustomer - runningCost
            if (profit > maxProfit) {
                maxProfit = profit
                ans = time
            }
