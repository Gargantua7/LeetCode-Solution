...
class Solution1744 {
    fun canEat(candiesCount: IntArray, queries: Array<IntArray>): BooleanArray {
        ...
        val sum = LongArray(n)
        sum[0] = candiesCount[0].toLong()
        (1 until n).forEach {
            sum[it] = sum[it - 1] + candiesCount[it]
        }

        return BooleanArray(queries.size).apply {
            indices.forEach {
                val (type, day, max) = queries[it]
                val x1 = day.toLong() + 1
                val y1 = (day + 1).toLong() * max
                val x2 = if (type == 0) 1 else sum[type - 1] + 1.toLong()
                val y2 = sum[type]
