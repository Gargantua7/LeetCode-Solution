...
class DisplayTableOfFoodOrdersInARestaurant {
    ...
    class Solution {
        fun displayTable(orders: List<List<String>>): List<List<String>> {
            ...
            val maps: HashMap<Int, Map<String, Int>> = HashMap()
            orders.forEach { (_, id, food) ->
                set.add(food)
                val map = (maps[id.toInt()] ?: HashMap()).toMutableMap()
                map[food] = (map[food] ?: 0) + 1
                maps[id.toInt()] = map
            }

            val foodSortList = set.sorted()
            val tableIdSortList = maps.keys.sorted()
            ...
            return ArrayList<List<String>>().apply {
                ...
                tableIdSortList.forEach { id ->
                    val map = maps[id] ?: HashMap()
                    add(ArrayList<String>().apply {
                        add(id.toString())
                        foodSortList.forEach { name ->
                            add((map[name] ?: 0).toString())
                        }
                    })
                }
            }
