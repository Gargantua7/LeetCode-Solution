...
// 
// Related Topics 几何 数组 数学 排序 滑动窗口 👍 69 👎 0
...
class MaximumNumberOfVisiblePoints {
    ...
    class Solution {
        fun visiblePoints(points: List<List<Int>>, angle: Int, location: List<Int>): Int {
            val (u, v) = location
            var same = 0
            val polar = ArrayList<Double>()
                points.forEach { (i, j) ->
                val x = (i - u).toDouble()
                val y = (j - v).toDouble()
                if (x == 0.0 && y == 0.0) same++
                else polar += kotlin.math.atan2(y, x) / kotlin.math.PI * 180
            }
            polar.sort()
            polar += polar.filter { it <= -180 + angle }.map { it + 360 }
            val queue: Queue<Double> = LinkedList()
            var max = 0
            //println(polar.toDoubleArray().contentToString())
            polar.forEach { a ->
                queue.offer(a)
                if (queue.peek() < a - angle) queue.poll()
                max = max.coerceAtLeast(queue.size)
            }
            return max + same
        }
    }
