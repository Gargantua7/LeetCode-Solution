...
// 0 <= special[i][j] <= 50 
// 
// Related Topics 位运算 记忆化搜索 数组 动态规划 回溯 状态压缩 👍 192 👎 0
...
class ShoppingOffers {
    ...
    class Solution {
        ...
        var memo = HashMap<List<Int>, Int>()

        fun shoppingOffers(price: List<Int>, special: List<List<Int>>, needs: List<Int>): Int {
            val filter = special.filter { sp ->
                sp.sum() > sp.last() && price.foldIndexed(0) { i, ans, n -> ans + n * sp[i] } > sp.last()
            }
            return dfs(price, needs, filter)
        }

        fun dfs(price: List<Int>, needs: List<Int>, special: List<List<Int>>): Int {
            if (needs !in memo) {
                memo[needs] = special.fold(price.foldIndexed(0) { i, ans, n -> ans + needs[i] * n }) { min, sp ->
                    val spPrice = sp.last()
                    ...
                    if (next.size == price.size) min.coerceAtMost(dfs(price, next, special) + spPrice) else min
                }
            }
            return memo[needs]!!
        }
