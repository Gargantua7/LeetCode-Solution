...
class Solution {
    fun maxIncreaseKeepingSkyline(grid: Array<IntArray>): Int {
        val maxRow = IntArray(grid.size)
        val maxCol = IntArray(grid.size)
        for (i in grid.indices) {
            for (j in grid.indices) {
                maxRow[i] = maxRow[i].coerceAtLeast(grid[i][j])
                maxCol[j] = maxCol[j].coerceAtLeast(grid[i][j])
            }
        }
        var ans = 0
        for (i in grid.indices) {
            for (j in grid.indices) {
                ans += minOf(maxRow[i], maxCol[j]) - grid[i][j]
            }
        }
        return ans
    }
