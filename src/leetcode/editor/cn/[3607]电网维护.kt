...
// 
//
// Related Topics 深度优先搜索 广度优先搜索 并查集 图 数组 哈希表 有序集合 堆（优先队列） 👍 39 👎 0
...
class Solution {
    fun processQueries(c: Int, connections: Array<IntArray>, queries: Array<IntArray>): IntArray {
        val group = ArrayList<TreeSet<Int>>()
        val map = HashMap<Int, Int>()
        val offline = HashSet<Int>()

        val edge = HashMap<Int, ArrayList<Int>>()
        connections.forEach { (s, e) ->
            edge.getOrPut(s, ::ArrayList).add(e)
            edge.getOrPut(e, ::ArrayList).add(s)
        }
        ...
        edge.forEach { (node, _) ->
            if (node in found) return@forEach
            val set = TreeSet<Int>().also { group.add(it) }
            map[node] = group.lastIndex
            queue.addFirst(node)

            while (queue.isNotEmpty()) {
                val curr = queue.removeFirst()

                if (curr in found) continue
                found.add(curr)
                set.add(curr)
                map[curr] = group.lastIndex

                edge[curr]?.forEach(queue::add)
            }
        }

        return queries.mapNotNull { (op, pow) ->

            if (op == 1) {
                if (pow !in offline) pow else {
                    map[pow]?.let {
                        group[it].firstOrNull()
                    } ?: -1
                }
            } else {
                offline.add(pow)
                val index = map[pow] ?: return@mapNotNull null
                group[index].remove(pow)
                null
            }
        }.toIntArray()
    }
