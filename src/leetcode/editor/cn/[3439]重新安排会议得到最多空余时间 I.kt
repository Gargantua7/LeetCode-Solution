 ...
 * 2025-07-09 19:00:07
 */
private class RescheduleMeetingsForMaximumFreeTimeI {	
//leetcode submit region begin(Prohibit modification and deletion)
class Solution {
    fun maxFreeTime(eventTime: Int, k: Int, startTime: IntArray, endTime: IntArray): Int {
        val arr = IntArray(startTime.size + 1) { i ->
            when (i) {
                0 -> startTime[0]
                startTime.size -> eventTime - endTime.last()
                else -> startTime[i] - endTime[i - 1]
            }
        }

        var curr = (0..k).sumOf { arr[it] }
        var max = curr

        for (i in k + 1..arr.lastIndex) {

            curr = curr - arr[i - k - 1] + arr[i]
            max = maxOf(max, curr)
