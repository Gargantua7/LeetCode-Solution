...
//
// 
// 输入：numberOfUsers = 2, events = [["MESSAGE","10","id1 id0"],["OFFLINE","11","0
//"],["MESSAGE","71","HERE"]] 
// 
//
// 输出：[2,2] 
...
// 
//
// Related Topics 数组 数学 排序 模拟 👍 4 👎 0
...
class Solution {
    fun countMentions(numberOfUsers: Int, events: List<List<String>>): IntArray {
        val onlineTimes = IntArray(numberOfUsers)
        val ans = IntArray(numberOfUsers)

        events.sortedBy {
            it[1].toDouble() + if (it[0] == "OFFLINE") 0.0 else 0.5
        }.forEach {
            val event = it[0]
            val timestamp = it[1].toInt()
            if (event == "MESSAGE") {
                val mentions = it[2]
                when (mentions) {
                    "ALL" -> ans.indices
                    "HERE" -> ans.indices.filter { i ->
                        onlineTimes[i] <= timestamp
                    }
                    else -> {
                        mentions.split(' ').map { s ->
                            s.removePrefix("id").toInt()
                        }
                    }
                }.forEach { i -> ans[i]++ }
            } else {
                val id = it[2].toInt()
                onlineTimes[id] = timestamp + 60
            }
        }
