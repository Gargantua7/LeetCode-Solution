...
class Solution {
    ...
    fun countCollisions(directions: String): Int {
        if (directions.toSet().size == 1) return 0
        return directions.indexOfLast { it != 'R' } + 1 -
                directions.indexOfFirst { it != 'L' } -
                directions.count { it == 'S' }
    }

    fun countCollisions1(directions: String): Int {
        var rightCount = 0
        var leftStatus = 0
        var ans = 0
