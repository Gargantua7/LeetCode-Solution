...
// 
// Related Topics 数组 动态规划 前缀和 👍 20 👎 0
...
class FindGoodDaysToRobTheBank {
    ...
    class Solution {
        fun goodDaysToRobBank(security: IntArray, time: Int): List<Int> {
            val n = security.size
            val lastIndex = security.lastIndex
            val left = IntArray(n)
            val right = IntArray(n)
            for (i in 1..lastIndex) {
                left[i] = if (security[i] <= security[i - 1]) left[i - 1] + 1 else 0
                right[lastIndex - i] =
                    if (security[lastIndex - i + 1] >= security[lastIndex - i]) right[lastIndex - i + 1] + 1 else 0
            }
            return ArrayList<Int>().apply {
                for (i in time..lastIndex - time) {
                    if (left[i] >= time && right[i] >= time) add(i)
                }
            }
        }
    }
