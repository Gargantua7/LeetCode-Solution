...
// 
//
// Related Topics 脑筋急转弯 数组 数学 组合数学 👍 28 👎 0
...
class Solution {
    fun countPermutations(complexity: IntArray): Int {
        if (complexity.slice(1..complexity.lastIndex).any { it <= complexity.first() }) return 0
        return (1..complexity.lastIndex).fold(1L) { acc, i -> acc * i % 1000000007L }.toInt()
    }
}
