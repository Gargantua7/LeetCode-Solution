...
// 
//
// Related Topics 数组 动态规划 排序 👍 140 👎 0
...
class Solution {
    fun bestTeamScore(scores: IntArray, ages: IntArray): Int {
        val src = scores.indices.map { scores[it] to ages[it] }.sortedWith(Comparator { (s1, a1), (s2, a2) ->
            if (s1 == s2) a1 - a2 else  s1 - s2
        })
        val dp = IntArray(scores.size)
        for (i in 0..dp.lastIndex) {
            val (currScore, currAge) = src[i]
            var prev = 0
            for (j in i - 1 downTo  0) {
                if (src[j].second <= currAge) {
                    prev = maxOf(dp[j], prev)
                }
            }
            dp[i] = currScore + prev
        }
