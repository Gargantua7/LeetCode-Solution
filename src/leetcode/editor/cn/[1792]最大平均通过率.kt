...
// 
//
// Related Topics 贪心 数组 堆（优先队列） 👍 65 👎 0
...
class Solution {
    fun maxAverageRatio(classes: Array<IntArray>, extraStudents: Int): Double {
        val queue = PriorityQueue<Pair<Pair<Int, Int>, Double>> { (_, a), (_, b) -> b.compareTo(a) }
        classes.forEach { (pass, total) ->
            queue.offer(pass to total to (total - pass).toDouble() / (total.toLong() * total - total))
        }
        repeat(extraStudents) {
            var (pass, total) = queue.poll().first
            queue.offer(++pass to ++total to (total - pass).toDouble() / (total.toLong() * total - total))
        }
        return queue.fold(0.0) { arr, (clazz , _) -> arr + clazz.first.toDouble() / clazz.second } / queue.size
    }
}
