...
//
// 
//进阶：找到每次 fetch 的复杂度为 O(n) 的算法比较简单。你可以找到每次 fetch 的复杂度更佳的算法吗？ Related Topics 栈 设计
 ...
 * 2021-12-04 14:51:34
 */
class DesignMostRecentlyUsedQueue {
...
class MRUQueue(n: Int) {
    ...
    init {
        queue = LinkedList((1..n).toList())
    }

    fun fetch(k: Int): Int {
        val n = queue.removeAt(k - 1)
        queue.add(n)
        return n
    }
