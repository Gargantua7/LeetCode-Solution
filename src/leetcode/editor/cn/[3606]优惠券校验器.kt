...
class Solution {
    fun validateCoupons(code: Array<String>, businessLine: Array<String>, isActive: BooleanArray): List<String> {
        ...
        val business = listOf("electronics", "grocery", "pharmacy", "restaurant")

        return code.zip(businessLine).filterIndexed { index, (c, b) ->
            c.isNotEmpty() && c.all { it in chars } && b in business && isActive[index]
        }.sortedBy { (c, b) -> "" + business.indexOf(b) + c }.map { it.first }
    }
}
