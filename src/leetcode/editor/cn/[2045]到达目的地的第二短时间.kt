...
// 1 <= time, change <= 10³ 
// 
// Related Topics 广度优先搜索 图 最短路 👍 30 👎 0
...
class Solution {
    fun secondMinimum(n: Int, edges: Array<IntArray>, time: Int, change: Int): Int {
        ...
        edges.forEach { (s, e) ->
            ...
            adjacency.getOrPut(e, ::ArrayList).add(s)
        }
        val path = Array(n + 1) { IntArray(2) { Int.MAX_VALUE } }
        path[1][0] = 0
        val queue = LinkedList<IntArray>()
        queue.offer(intArrayOf(1, 0))
        while (path.last().last() == Int.MAX_VALUE) {
            val (curr, len) = queue.poll()
            adjacency[curr]?.forEach { next ->
                if (len + 1 < path[next][0]) {
                    path[next][0] = len + 1
                    queue.offer(intArrayOf(next, len + 1))
