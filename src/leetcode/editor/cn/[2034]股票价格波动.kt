...
class StockPrice {
    ...
    val box = TreeMap<Int, Int>()
    var currTime = 0

    fun update(timestamp: Int, price: Int) {
        if (timestamp in map) {
            val k = map[timestamp]!!
            if (box[k] == 1) box.remove(k)
            else box[k] = box[k]!! - 1
        }
        map[timestamp] = price
        box[price] = (box[price]?: 0) + 1
        currTime = currTime.coerceAtLeast(timestamp)
    }

    fun current(): Int {
        return map[currTime]?: 0
    }

    fun maximum(): Int {
        return box.floorKey(Int.MAX_VALUE)
    }

    fun minimum(): Int {
        return box.ceilingKey(0)
    }
