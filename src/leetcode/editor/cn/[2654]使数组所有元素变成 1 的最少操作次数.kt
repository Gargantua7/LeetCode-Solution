...
class Solution {
    fun minOperations(nums: IntArray): Int {
        val oneCounts = nums.count { it == 1 }
        if (oneCounts > 0) return nums.size - oneCounts

        val allGcd = nums.fold(nums.first(), ::gcd)
        if (allGcd != 1) return -1

        val list = nums.toList()
        for (len in 2..nums.size) {
            for (start in 0..nums.size - len) {
                val gcd = list.subList(start, start + len).fold(nums[start], ::gcd)
                if (gcd == 1) {
