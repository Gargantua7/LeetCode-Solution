...
class Solution {
    fun categorizeBox(length: Int, width: Int, height: Int, mass: Int): String {
        return when {
            isBulky(length, width, height) && isHeavy(mass) -> "Both"
            isBulky(length, width, height) -> "Bulky"
            isHeavy(mass) -> "Heavy"
            else -> "Neither"
        }
    }
    ...
    fun isBulky(length: Int, width: Int, height: Int): Boolean =
            ...
            width >= 10_000 ||
            height >= 10_000 ||
            length.toLong() * width * height >= 1000_000_000

    fun isHeavy(mass: Int): Boolean = mass >= 100
}
