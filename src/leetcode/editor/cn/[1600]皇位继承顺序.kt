...
class ThroneInheritance {
    ...
    class ThroneInheritance(val kingName: String) {
        ...
        fun birth(parentName: String, childName: String) {
            val children = edges[parentName]?: ArrayList()
        ...
        fun getInheritanceOrder(): List<String> {
            ...
            while(stack.isNotEmpty()) {
                val p = stack.pop()
                if(p !in dead) list.add(p)
                val children = edges[p]?: ArrayList()
                (children.lastIndex downTo 0).forEach { i -> stack.push(children[i]) }
            }
            return list
        }
